    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Saldo disponible", value: `$${wallet?.balance?.toFixed(2) || "0.00"}`, icon: DollarSign, color: "text-orange-600", bg: "bg-orange-50" },
            { label: "Solicitudes", value: pendingSessions.length, icon: Clock, color: "text-yellow-600", bg: "bg-yellow-50" },
            { label: "Confirmadas", value: confirmedSessions.length, icon: Calendar, color: "text-blue-600", bg: "bg-blue-50" },
            { label: "Completadas", value: completedCount, icon: CheckCircle, color: "text-green-600", bg: "bg-green-50" },
          ].map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            {pendingSessions.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-6">No hay solicitudes pendientes</p>
            ) : (
              <div className="space-y-3">
                {pendingSessions.map((session) => {
                  const [year, month, day] = session.date.split("T")[0].split("-").map(Number);
                  const date = new Date(year, month - 1, day).toLocaleDateString("es-HN", { weekday: "short", month: "short", day: "numeric" });
                  return (
                    <div key={session.id} className="border border-gray-100 rounded-lg p-4">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-700">{session.student.name}</p>
                          <p className="text-xs text-gray-400">{session.subject.name} · {date} {session.startTime}</p>
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            {confirmedSessions.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-6">No hay sesiones confirmadas</p>
            ) : (
              <div className="space-y-3">
                  ...
                  return (
                    <div key={session.id} className="border border-gray-100 rounded-lg p-4">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className="text-orange-600 font-semibold text-sm">${session.price}</span>
                      </div>
                      <div className="flex gap-2">
                        {session.meetingUrl && (
                          <a
          ...
          {/* Wallet summary & Quick actions... (sin cambios) */}
